...
class MSTracker(Tracker):
    ...
    def initialize(self, image, region):
        ...
        self.q = extract_histogram(patch, self.bins, self.kernel)
    ...
    def track(self, image):
        for i in range(20):
            ...
            p = extract_histogram(patch, self.bins, self.kernel) * self.c
            p = p/np.sum(p)

            backproject = backproject_histogram(patch, np.sqrt(self.q/(p+self.eps)), self.bins)
            ...
            if int(np.sqrt(x_shift**2 + y_shift**2)) <= self.stop_dist:
                break
    ...
    def get_background_info(self, image):
        ...
        background_hist = extract_histogram(background_patch, self.bins, create_epanechnik_kernel(self.size[0]*3, self.size[1]*3, self.sigma)) - self.q
    ...
    def update_q(self, patch):
        new_q = extract_histogram(patch, self.bins, self.kernel) * self.c
        new_q = new_q/np.sum(new_q)
        self.q = (1-self.alpha) * self.q + self.alpha * new_q
