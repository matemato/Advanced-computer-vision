...
from particle_tracker_2 import ParticleTracker
...
while frame_idx < sequence.length():
    ...
    if frame_idx == init_frame:
        ...
        t_ = time.time()
        particles, weights = tracker.initialize(img, sequence.get_annotation(frame_idx, type='rectangle'))
        ...
        t_ = time.time()
        predicted_bbox, particles, weights = tracker.track(img)
        time_all += time.time() - t_
